...
$atua_total_pedido = mysql_query("UPDATE pedido_cle2012 SET `status` = '$var_pedido'  WHERE  `npedido` = '$npedido'"); 
}
$query_1 = mysql_query("SELECT * FROM pedido_cle2012 WHERE npedido = '$npedido'") or die(mysql_error());
#-----------------------------------------------------------------------------------------------
...
$corpo_npedido = mysql_query("SELECT * FROM login WHERE `B` = '$emxls_cliente'"); 
	while($array_emp = mysql_fetch_array($corpo_npedido)){		
	  $emp_loja   = $array_emp["loja"];	
		}
$corpo_npedido2 = mysql_query("SELECT * FROM login WHERE `B` = '$emxls_cliente_filho'"); 
...
//consulta sql
$SQL = "SELECT * FROM `lista_artigo_pedido_cle2012` WHERE npedido = '$npedido' ORDER BY data, artigo ";
