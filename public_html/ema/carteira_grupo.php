...
<table width="100%" class="pading">
   ...
   <tr bgcolor="#FF0000">
     <td>Categoria</td><td>Cód</td><td>Cliente</td><td>Cidade</td><td>Coleção</td><td>Artigo</td><td>Descrição</td><td>Qt</td><td>Total</td><td>Quantidade por tamanho</td><td>Entrega Revisada</td><td>Status</td><td>N Pedido</td>
   </tr>
...
$query2 = mysql_query("select * FROM `carteira` INNER JOIN `login` ON `carteira`.`Codigo Cliente` = `login`.`B` WHERE `login`.`nome` = '$grupo' AND `Data Entrega  Revisada` BETWEEN '2011-01-01' AND '2011-12-30' ORDER BY `Codigo Cliente` , `Data Entrega  Revisada`, `Descricao Artigo` ") or die(mysql_error());
...
echo "
 ...
 <tr bgcolor=\"$cor\"  >
   <td>&nbsp;$categoria</td><td>&nbsp;$ccodigo_cliente</td><td>&nbsp;$cliente_grupo</td><td>&nbsp;$cidade</td><td>$colecao</td><td><a href=\"javascript:muestra_imagen('thumb/".$cod_Artigo."_F.jpg',300,210)\">$cod_Artigo Ver foto</a></td><td>&nbsp;$desc_artigo</td><td>$qtnd_faturada</td><td>R$ $total_faturado</td><td>$qtnd_tamanho</td><td>$entrega_revisada</td><td>$status</td><td>$n_pedido_ca</td>
 </tr>";
 $cont++;
