...
$query_pedido_aberto = mysql_query("SELECT DISTINCT artigo, data FROM  lista_artigo_pedido_cle2012  WHERE npedido = '$p_pedido'") or die(mysql_error());
...
$query_total = mysql_query("SELECT SUM(quantidade) as soma FROM  lista_artigo_pedido_cle2012  WHERE `artigo` = '$sqlpedido_artigo' AND `npedido` = '$p_pedido' AND `data` = '$sqlpedido_data'") or die(mysql_error());
while($linha_array = mysql_fetch_array($query_total)){$linha = $linha_array["soma"];}

#------------------------------------------------------------
#----------------------Soma Pedido---------------------------

$query_total_pe = mysql_query("SELECT SUM(total) as soma FROM  lista_artigo_pedido_cle2012  WHERE  `npedido` = '$p_pedido'") or die(mysql_error());
while($linha_array_pe = mysql_fetch_array($query_total_pe)){$linha_pe = $linha_array_pe["soma"];}

#------------------------------------------------------------
#------recolhendo informações------------------------
$query_info = mysql_query("SELECT * FROM  lista_artigo_pedido_cle2012  WHERE `artigo` = '$sqlpedido_artigo' AND `npedido` = '$p_pedido'") or die(mysql_error());
while($info_array = mysql_fetch_array($query_info)){$info_descricao = $info_array["descricao"];$info_valor_unite = $info_array["valor"];$info_valor_segm = $info_array["segmento"];}
...
#-----------Tratanto valores e fazendo conta -------------------
	...
	$total_item        = $info_valor_unite * $linha;
...
#------------------------------------------------------------------
	...
	echo "<tr><td><img src=\"http://www.adisul.com/thumb/".$sqlpedido_artigo."_F.jpg\" width=\"60px\" style=\"background:none;\" /></td><td>$sqlpedido_artigo</td><td>$info_descricao</td><td>$linha</td><td>$info_valor_segm</td><td>R$ $info_valor_unite</td><td>R$ $total_item</td><td>$sqlpedido_data</td><td>|</td></tr>
