...
$query_pedido_aberto = mysql_query("SELECT DISTINCT artigo, data FROM  lista_artigo_pedido_cle_1  WHERE npedido = '$p_pedido'") or die(mysql_error());
...
$query_total = mysql_query("SELECT SUM(quantidade) as soma FROM  lista_artigo_pedido_cle_1  WHERE `artigo` = '$sqlpedido_artigo' AND `npedido` = '$p_pedido' AND `data` = '$sqlpedido_data'") or die(mysql_error());
...
$query_total_pe = mysql_query("SELECT SUM(total) as soma FROM  lista_artigo_pedido_cle_1  WHERE  `npedido` = '$p_pedido'") or die(mysql_error());
while($linha_array_pe = mysql_fetch_array($query_total_pe)){$linha_pe = $linha_array_pe["soma"];}

#------------------------------------------------------------
#----------------------Recolhendo Informações-----------------

$query_total_info = mysql_query("SELECT * FROM  lista_artigo_pedido_cle_1  WHERE  `npedido` = '$p_pedido' AND `artigo` = '$sqlpedido_artigo'") or die(mysql_error());
...
#$atua_total_pedido = mysql_query("UPDATE pedido_cle_1 SET `total` = '$total_pedido'  WHERE  `npedido` = '$p_pedido'"); 



if($total_pedido <= 5000) { $condicao = "menor";
												
							$ver_total = "<b class=\"$condicao\">Total do Pedido Nº $p_pedido R$ ".$total_pedido =  number_format($total_pedido, 2, ',', '.') ."</b>
							<br /> Somente será aceito pedido com valor mínimo de R$ 5.000,00<br />
									";} 
					else {
						$condicao = "maior";
