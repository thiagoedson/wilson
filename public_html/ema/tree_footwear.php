...
<div id="tree">    
  <div id="menu_tree">
  <a href="tree_footwear.php?filtro=venda" class="new_tree" style="background-color:#009;">Calçado</a>
  <a href="tree_apparel.php?filtro=venda" class="new_tree">Têxtil</a>
...
Escolha o Filtro: <select name="filtro">
  ...
  <table class="tabela">
    <tr class="realceLinha">
      <td class="coluna11">Produto</td>
      <td class="coluna7">Artigo</td>
      <td class="coluna6">Descrição</td>
      <td class="coluna5">Estoque<br /> mês <br />anterior</td>
      <td class="coluna4">Vendas <br /> mês <br />anterior</td>
      <td class="coluna3">Quantidde/<br /> Carteira</td>
      <td class="coluna2">Próximo<br />recebimento</td>
      <td class="coluna1">Data <br /> último <br />Faturamento</td>
      <td class="coluna9">Repor<br />Agora</td>
      <td class="coluna10">Promocional<br />Disponível</td>
    </tr>
...
if (($i % 2) == 1){ $fundo="white"; }else{ $fundo="#F3F3F3"; }
	  ...
	  $descricao    = $array_tree["Descricao"];
	...
	$query_tree_2 = mysql_query("SELECT `Data Emissao NF`,`Order Type` FROM `$banco_representante`.`faturamento` INNER JOIN `$banco_representante`.`login` ON `faturamento`.`Codigo Cliente` = `login`.`B` WHERE `login`.`nome` = '$grupo' AND `Codigo Artigo` = '$artigo' ORDER BY `Data Emissao NF` DESC LIMIT 1");
	
	
    while($array_tree_2 = mysql_fetch_array($query_tree_2)){	
	  $qt_faturada       = $array_tree_2["Data Emissao NF"];
	  $qt_ordertype       = $array_tree_2["Order Type"];  
	  
	  if($qt_ordertype == "CL2") {$qt_ordertype = "Promocional";}
	  elseif($qt_ordertype == "CLO") {$qt_ordertype = "Promocional";}
	  else {$qt_ordertype = "";}
	...
	if($qt_loja <> "") $qt_loja = "<a href=\"shopwide/nss13/shop.php?artigo=".$artigo."\" title=\"Produto Disponível para Pronta-entrega\" target=\"_blank\">Comprar</a>";
	else {$qt_loja = "";}
	...
	if($qt_lojaa <> "") $qt_lojaa = "<a href=\"shopwide/promo/shop.php?artigo=".$artigo."\" title=\"Produto Disponível no Promocional CLE\" target=\"_blank\">Comprar</a>";
	else {$qt_lojaa = "";}
	...
	echo "<tr  class=\"realceLinha\">
			<td class=\"coluna11\"><img src=\"http://adisul.net/demandimage/thumb/".$artigo."_F.jpg\" width=\"35px\" height=\"25px\" /></td>
			<td class=\"coluna7\">".$artigo."<br />
	        $qt_tabela</td>
			<td class=\"coluna6\">".$descricao."</td>
			<td class=\"coluna5\">".$estoque."</td>
			<td class=\"coluna4\">".$vendas."</td>
			<td class=\"coluna3\">".$qt_carteira."</td>
			<td class=\"coluna2\">".$data_carteira."</td>
			<td class=\"coluna1\">".$qt_faturada."<br/> ".$qt_ordertype."</td>
			<td class=\"coluna9\">".$qt_loja."</td>
			<td class=\"coluna10\">".$qt_lojaa."</td>
		</tr>";
