...
$query_pedido_aberto = mysql_query("SELECT DISTINCT artigo, data FROM  lista_artigo_pedido  WHERE npedido = '$p_pedido'") or die(mysql_error());
...
$query_total = mysql_query("SELECT SUM(quantidade) as soma FROM  lista_artigo_pedido  WHERE `artigo` = '$sqlpedido_artigo' AND `npedido` = '$p_pedido' AND `data` = '$sqlpedido_data'") or die(mysql_error());
...
$query_total_pe = mysql_query("SELECT SUM(total) as soma FROM  lista_artigo_pedido  WHERE  `npedido` = '$p_pedido'") or die(mysql_error());
while($linha_array_pe = mysql_fetch_array($query_total_pe)){$linha_pe = $linha_array_pe["soma"];}

#------------------------------------------------------------
#----------------------Recolhendo Informações-----------------

$query_total_info = mysql_query("SELECT * FROM  lista_artigo_pedido  WHERE  `npedido` = '$p_pedido' AND `artigo` = '$sqlpedido_artigo'") or die(mysql_error());
...
#------------------------------------------------------------------
	...
	echo "<tr><td><img src=\"http://www.adisul.com/thumb/".$sqlpedido_artigo."_F.jpg\" width=\"60px\" style=\"background:none;\" /></td><td>$sqlpedido_artigo</td><td>$info_descricao_unite</td><td>$linha</td><td>$info_segmento_unite</td><td>R$ $info_valor_unite</td><td>R$ $total_item </td><td>$sqlpedido_data</td><td>|</td><td>$status_artigo</td></tr>
