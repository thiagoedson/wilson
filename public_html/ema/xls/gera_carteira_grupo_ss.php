...
//consulta sql
$SQL = "select * FROM `carteira` INNER JOIN `login` ON `carteira`.`Codigo Cliente` = `login`.`B` WHERE `login`.`nome` = '$grupo' ORDER BY `Codigo Cliente` , `Data Entrega  Revisada`, `Descricao Artigo` ";
...
$corpo_npedido = mysql_query("SELECT * FROM `login` WHERE `B` = '$senha'"); 
	while($array_emp = mysql_fetch_array($corpo_npedido)){		
	 ...
	 $represetante_sql = mysql_query("SELECT * FROM `zk10n546_adidas`.`usuarios` WHERE `id` = '$e_represetante'"); 
...
$carteira_site = date('d/m/Y', strtotime(str_replace("-", "/", $carteira_site )));
   ...
   $headers .= 'From: staff@adisul.net' . "\r\n" .
               'Reply-To: staff@adisul.net , '.$emailrepre.' ,' . "\r\n" .
