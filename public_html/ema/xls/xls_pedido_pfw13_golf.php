...
$var_pedido = $_GET["var"];
$sul = 1;
$query_1 = mysql_query("SELECT * FROM pedido_pfw13_golf WHERE npedido = '$npedido'") or die(mysql_error());
#-----------------------------------------------------------------------------------------------
...
// montando a tabela
echo "<h1>$texto_cond<br />
Nº pedido <b style=\"color:#F00\"> $npedido</b></h1>";
...
$query_car = mysql_query("SELECT DISTINCT `loja` FROM  `lista_artigo_pedido_pfw13_golf`  WHERE `npedido` = '$npedido'");
...
$somar_total_pedido_1 = mysql_query("SELECT SUM(total) as soma FROM  `lista_artigo_pedido_pfw13_golf`  WHERE  `npedido` = '$p_pedido' AND `loja` = '$car_loja'") or die(mysql_error());
...
$linha_eee          =  number_format($linha_eee, 2, ',', '.');
	...
	echo "<tr><td colspan=\"10\">$car_loja - <b>$car_nome_loja</b> | <b style=\"color:#F00\">$car_refere</b> | Total desta loja* R$ $linha_eee</h3></td></tr>";
	
$query_pedido_aberto = mysql_query("SELECT DISTINCT `artigo`, `data` FROM  `lista_artigo_pedido_pfw13_golf`  WHERE `npedido` = '$npedido' AND `loja` = '$car_loja' ORDER BY `loja`,`data`") or die(mysql_error());	
	$lin = mysql_num_rows($query_pedido_aberto);
    if($lin==0) // verifica se retornou algum registro
    {
		$atua_total_pedido_zero = mysql_query("UPDATE `pedido_pfw13_golf` SET `total` = '$zero'  WHERE  `npedido` = '$p_pedido'"); 
...
$query_total = mysql_query("SELECT SUM(quantidade) as soma FROM  `lista_artigo_pedido_pfw13_golf`  WHERE `artigo` = '$sqlpedido_artigo' AND `npedido` = '$p_pedido' AND `data` = '$sqlpedido_data' AND `loja` = '$car_loja'") or die(mysql_error());
while($linha_array = mysql_fetch_array($query_total)){$linha = $linha_array["soma"];}
#------------------------------------------------------------
#----------------------Soma Pedido---------------------------
$query_total_pe = mysql_query("SELECT SUM(total) as soma FROM  `lista_artigo_pedido_pfw13_golf`  WHERE  `npedido` = '$p_pedido' AND `loja` = '$car_loja'") or die(mysql_error());
while($linha_array_pe = mysql_fetch_array($query_total_pe)){$linha_pe = $linha_array_pe["soma"];}
#------------------------------------------------------------
#----------------------Recolhendo Informações-----------------
$query_total_info = mysql_query("SELECT * FROM  `lista_artigo_pedido_pfw13_golf`  WHERE  `npedido` = '$p_pedido' AND `artigo` = '$sqlpedido_artigo' AND `loja` = '$car_loja' AND `data` = '$sqlpedido_datax'") or die(mysql_error());
...
#-----------Tratanto valores e fazendo conta -------------------  	
	...
	$query_tam_sql = mysql_query("SELECT `tamanho`,`quantidade` FROM `lista_artigo_pedido_pfw13_golf` WHERE `npedido` = '$p_pedido' AND `artigo` = '$sqlpedido_artigo' AND `loja` = '$car_loja' AND `data` = '$sqlpedido_datax'")or die(mysql_error());
