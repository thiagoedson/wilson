...
header("Content-Disposition: attachment; filename=\"$npedido-$emxls_cliente-adisul_pedido_ss16_$nome_cliente.doc\";");
?>
...
if($texto_cond == "3") {$texto_cond = "CÓPIA DO PEDIDO EM ABERTO SS16";}
                  else {$texto_cond = "PEDIDO PRÉ-VENDA SS16";}
...
$query_pedido_aberto = mysqli_query($con, "SELECT DISTINCT `artigo`, `data` FROM  `lista_artigo_pedido_ss16`  WHERE `npedido` = '$npedido' AND `loja` = '$car_loja' ORDER BY `loja`,`data`") or die(mysql_error());	
...
#------------------------------------------------------------------	
	echo "<tr> 
	       <td>$car_loja</td>
		   <td>$car_refere</td>
		   <td><img src=\"http://www.adisul.net/demandimage/rbk/thumb/".$sqlpedido_artigo."_F.jpg\" width=\"40px\" style=\"max-width:40px;max-height:40px;\" /></td>
		   <td></td>
