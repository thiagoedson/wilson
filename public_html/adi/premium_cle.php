...
#--------------------------- Funcões para clientes premium_cles-----------------------

function addCliente() {
	
	$cd_cl = $_GET["cod_cl"];
	$ssql = mysqli_query($con,"UPDATE  tb_cle12 SET `typt` = '' WHERE  `cliente` = '$cd_cl'") or die(mysql_error());
	header("location:premium_cle.php?$cd_cl&msg=1");
}

function remCliente() {
	
	$cd_cl = $_GET["cod_cl"];
	$ssql = mysqli_query($con,"UPDATE  `tb_cle12` SET `typt` = '1' WHERE  `cliente` = '$cd_cl'") or die(mysql_error());
	header("location:premium_cle.php?$cd_cl&msg=2");
}

function addCliente_tb() {
	
	$cd_cl = $_POST["cod_adidas"];
    $resultado_npedido = mysqli_query($con,"INSERT INTO `tb_cle12` (`id`, `cliente`, `typt`) VALUES (NULL, '$cd_cl', '')") or die(mysql_error());
	header("location:premium_cle.php?cadastrado");
}
#-------------------------------------------------------------------------------------------------------------------------------------------
...
function addGrupo() {
	...
	$ssql = mysqli_query($con,"UPDATE  tb_cle12 SET `typt` = '' WHERE  `cliente` = '$nome_cliente_fx_2'") or die(mysql_error());
	
	}
	header("location:premium_cle.php?$cd_cl&msg=456");
}
...
function remGrupo() {
	...
	$nome_cliente_fx_2         = $array_fx_2["B"];
	$ssql = mysqli_query($con,"UPDATE  tb_cle12 SET `typt` = '1' WHERE  `cliente` = '$nome_cliente_fx_2'") or die(mysql_error());
	
	}
	header("location:premium_cle.php?$cd_cl&msg=555");
}
...
<?php
$query_fx = mysqli_query($con,"SELECT * FROM `$banco`.`tb_cle12` WHERE `typt` = '' ORDER BY `cliente` ASC") or die(mysql_error());
...
<?php
$query_fx = mysqli_query($con,"SELECT * FROM `$banco`.`tb_cle12` WHERE `typt` = '1' ORDER BY `cliente` ASC") or die(mysql_error());
while($array_fx = mysqli_fetch_assoc($query_fx)){	
	$cliente_fx         = $array_fx["cliente"];
